...
class RESTJsonDataSource extends DataSource {
  ...
  def executeQuery(ds: DataSet, query: DataSet): DataSet = {
    ...
    if (uri.isDefined) {
      ...
      val passwordOption = query("credential")("password").stringOption.orElse(ds("credential")("password").stringOption)
      ...
      val otherHeaders = (
        if(query("headers").isDefined)
          query("headers")
        else
          ds("headers"))
        .map(h => new BasicHeader(h.label, h.stringOption.getOrElse(""))).toList
